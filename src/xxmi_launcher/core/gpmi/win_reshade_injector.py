"""Small Windows-only DLL injector for GPMI.

This replaces the old dependency on XXMI's 3dmloader.dll.  It starts the
selected Godot executable suspended, loads dxgi.dll into the process via
LoadLibraryW, then resumes the main thread. It also supports per-child
environment overrides, which ReShade needs when loaded as dxgi.dll rather
than as a graphics API proxy DLL such as dxgi.dll.
"""
...
def _kernel32():
    if os.name != "nt":
        raise GPMIInjectionError("GPMI ReShade injection is only available on Windows.")
    ...
    k32.CreateProcessW.argtypes = [
        ...
        wt.LPVOID,
        wt.LPCWSTR,
        ct.POINTER(STARTUPINFOW),
        ct.POINTER(PROCESS_INFORMATION),
    ]
...
def _make_environment_block(overrides: Mapping[str, str] | None) -> ct.Array:
    """Return a Windows UTF-16 environment block for CreateProcessW.

    Passing an explicit environment is safer than temporarily mutating
    os.environ in the launcher process. ReShade uses these variables very early
    in DllMain, so they must be present before the remote LoadLibraryW call.
    """
    ...
    # Windows requires a case-insensitively sorted block of null-terminated
    # key=value strings, followed by one extra null terminator.
...
def _inject_load_library_w(k32, process_handle: wt.HANDLE, dll_path: Path, timeout_ms: int) -> None:
    ...
    dll_buffer = ct.create_unicode_buffer(dll_text)
    ...
    try:
        ...
        if exit_code.value == 0 or exit_code.value == STILL_ACTIVE:
            raise GPMIInjectionError(
                f"LoadLibraryW failed for {dll_path}. "
                "ReShade can return failure when it is loaded as dxgi.dll "
                "without a valid ReShade.ini/base path, or when architecture, "
                "dependency, permission, or security software checks block DLL loading."
            )
    finally:
        if remote_thread:
            k32.CloseHandle(remote_thread)
        # 0x8000 = MEM_RELEASE. dwSize must be 0 with MEM_RELEASE.
...
def start_suspended_and_inject_dll(
    ...
    """Start *exe_path*, inject *dll_path*, resume it, and return the process id."""
    ...
    creation_flags = CREATE_SUSPENDED | NORMAL_PRIORITY_CLASS | CREATE_UNICODE_ENVIRONMENT
    env_block = _make_environment_block(env_overrides) if env_overrides else None
